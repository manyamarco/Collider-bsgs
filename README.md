...
- **Linux:** For Linux, we provide a compilation script that automatically injects workarounds (like `-no-pie`) for modern distributions, ensuring PB 5.31 builds work properly. Make sure CUDA is available and set the environment variable pointing to the PureBasic Linux compiler directory:
```bash
cd PureBasic
export PUREBASIC_HOME="/path/to/purebasic"
chmod +x compilar.sh
./compilar.sh
```
